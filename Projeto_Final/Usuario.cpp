...
//FUNCOES DO USUARIO
void Usuario::criar_conta(std::string nome,std::string banco,int saldo){
    _contas.push_back(Conta(nome,banco,saldo));
}
void Usuario::excluir_conta(int i){
    _contas.erase(_contas.begin()+i);
}
void Usuario::criar_objetivo(std::string nome, std::string descricao, int valor_meta, std::string data_limite){
    _objetivos.push_back(Objetivo(nome, descricao, valor_meta, data_limite));
}
